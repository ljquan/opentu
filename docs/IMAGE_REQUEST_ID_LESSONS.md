...
- OpenTu 公网部署仍直连可信 Tuzi API，不新增匿名公网代理；浏览器提交会自动选用已验证可放行 `X-Request-Id` 的 `bus`、`bus2`、`bus3`、`business` 节点，并在节点网络失败时依次切换。
- 本机 Vite 开发继续使用既有同源代理；局域网页面和任意公网域名使用上述公网 CORS 节点，不依赖访问者来源域名白名单。
...
通过标准：任意公网或局域网 OpenTu 页面提交时，会直连已验证的 Tuzi CORS 节点，请求头等于本地任务 ID，并可在四个节点间故障切换；同一任务重试复用原 ID、不同任务 ID 不同、轮询 `GET` 不携带该头、非可信自定义供应商不携带该头。
