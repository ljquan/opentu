...
普通 `gpt-image-2.5`、VIP、Sunburst 和 Flare 显示“1K / 2K / 4K”，默认 1K，不再提供分辨率自动选项，历史自动偏好恢复为 1K。业务 1K 使用内部值 `billing-1k`，保留原自动档的请求方式；业务 2K、4K 分别保留内部 `1k`、`2k` 的请求方式。旧的内部 `4k` 选项移除，历史偏好恢复为内部 `2k`（界面 4K）。旧任务请求的内部档位仍可由适配器解析。该调整不改变 `gpt-image-2` 和固定 1k 型号；图片比例和画质中的自动选项保留。
