...
- [x] 12.3 合并有声片段并沿用最终素材库与原画板幂等交付
- [x] 12.4 在 UI 显示生成式语音、音色、时长和同步不稳定提示
- [ ] 12.5 覆盖单/双人提示词、逐页顺序、失败、取消、兼容降级和资源释放测试
  - 剩余：真实浏览器 MediaRecorder 取消与长任务资源释放 E2E
- [x] 11.8 主题生成新增独立 PPT，保留已有页面并按 jobId 隔离本次页面集合
- [x] 11.9 移除 PPT 讲解 Agent 底栏的音频模型及创建输入路由（原方案已废弃）
