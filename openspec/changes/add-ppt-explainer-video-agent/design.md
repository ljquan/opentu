...
- Goals:
  - 主题生成和当前 PPT 两种演示来源、两种审核策略、单人讲解和双人对谈完整可配置
...
- Non-Goals:
  ...
  - 不向新任务提供 PPTX 上传或解析入口；已持久化旧任务仅由兼容路径读取和恢复
...
flowchart LR
  ...
  MCP --> CHECK["画板、来源与缺图判定"]
  CHECK --> PLAN["现有模型路由与凭据预检"]
  PLAN --> ROOT["VIDEO 根任务 / IndexedDB"]
  ROOT --> SRC["主题大纲 / 当前 PPT 页面准备"]
  SRC --> REVIEW["确认大纲或警告后跳过"]
  REVIEW --> DECK["生成并写回可见 PPT 页面"]
  DECK --> SNAPSHOT["从同一页面冻结快照"]
  SNAPSHOT --> SCRIPT["页级讲稿与 speaker turns"]
  SCRIPT --> CLIPS["逐页生成有声视频片段"]
...
新任务不声明或伪造额外的语音、数字人或专用成片服务。编排器先复用既有 PPT 大纲与页面生图链路，使用用户选择的图片模型生成并写回 PPT 页面；再把每页结构化 speaker turns 写入提示词，串行调用用户已选的视频模型生成带语音片段。不传 `referenceImages`，以复用普通文生视频请求链路，避免把请求切换成供应商未必支持的图生视频。PPT 模型选择器直接使用运行时供应商目录，只展示当前启用供应商实际获取、勾选且具有可执行路由的图片与视频模型；不追加未获取或无路由的静态目录模型，也不再用静态音轨能力标签隐藏真实配置模型。

模型是否执行语音提示、能否生成可播放音轨以及朗读与时长是否稳定，由供应商真实结果决定。系统不得把模型名称、目录描述或本地标签包装成能力保证；缓存、媒体解码或播放失败时保留真实错误，并提示用户改用能够生成有声音轨的视频模型。用户设置的每页秒数是最长播放窗口，不是任意写入供应商请求的单段时长；编排器必须根据所选模型真实 `durationOptions` 将一页拆为一个或多个合法片段，并在片段存在溢出时只使用目标窗口内的媒体。供应商片段提前自然结束时直接使用实际时长，不补静音或延长页面。

生成式视频的视觉输出不是 PPT 固定画面，禁止直接拼接为最终成片。每页片段只作为讲解音轨来源：先强制缓存为同源 internal 媒体，再由独立本地合成器通过 HTMLMediaElement 和 Web Audio 播放音轨，Canvas 全程绘制原 PPT 快照、字幕和转场，最终只登记录制出的合成结果。字幕按讲稿的自然短句依次切换，每个时刻只展示当前 cue，不把整页讲稿长期叠在一张 PPT 上。双人模式必须按连续 speaker turns 拆成单角色片段，分别提交主讲人和嘉宾请求；主讲人固定使用成年男性、中低音约束，嘉宾固定使用成年女性、较高音约束，并按稳定的 `host`/`guest` 身份映射，不得依赖 speaker 数组顺序。该约束不等同于供应商 voice ID，视频模型最终是否精确执行仍由供应商决定；无音轨、缓存失败、无法解码或无法播放时保留真实错误，片段短于最长播放窗口时则直接交付实际时长。
...
```ts
type PptExplainerCreateSource = 'topic' | 'current_ppt';

interface PptExplainerTaskState {
  schemaVersion: 1;
  jobId: string;
  source: PptExplainerCreateSource | 'pptx'; // pptx 仅用于读取历史任务
...
`File`、`Blob`、base64、Authorization、API key、完整供应商响应和视频 chunks 禁止进入该状态。新建 DTO 只接受 `PptExplainerCreateSource`；持久化状态中的 `pptx` 仅用于识别、读取和恢复历史任务，不得通过 UI 或 MCP 创建。中间媒体只以任务私有缓存 URL 引用，并在取消、失败清理或完成后按所有权释放。

根任务保存每页目标秒数和可选讲解要求，旧任务缺少这些字段时继续使用历史兼容路径。根任务总时长保存为专用元数据或最终 `TaskResult.duration`，不得把整部成片总时长写入普通视频提交的 `GenerationParams.duration`；只有时间轴规划器产生的模型合法单段时长可以进入内部视频子任务，避免现有校验、模型固定时长和用户页目标相互混淆。
...
主题或当前 PPT 的缺失页面图不得仅作为编排器内部 override 使用。生成结果必须先复制为稳定画布媒体、更新 Frame 的 `slideImageUrl`/图片元素和页面版本，再由冻结器从画布读取；内部图片任务清理不得使已写回的 PPT 页面失效。本地合成器的 `imageUrl` 和用户可见 PPT 必须来自这次冻结的同一组页面，视频模型子任务不得携带页面 `referenceImages`。

每次任务计算 `deckFingerprint`，但不得用它阻止同一 PPT 并发任务。每个任务保留独立 `jobId`、页面顺序和快照 URL。

### Decision: 讲稿按页面和说话人结构化

讲稿生成以 `PPTFrameMeta.notes` 为主要依据，同时遵循用户的每页目标秒数和讲解要求。文本模型按自然普通话约每秒 4 个有效字符估算页级目标字数，备注不足以覆盖目标时长时围绕原意扩写，明显过长时保留重点并压缩；只有单人备注长度接近目标且没有额外讲解要求时才能直接复用而跳过模型。

- 单人模式每页至少一个 turn，speaker 固定为第一个人
- 双人模式每页优先生成一轮主讲人到嘉宾的连续发言，较长页面最多两轮；speaker 只能引用两个已配置 ID
- 时间轴按连续 speaker 边界拆分，同一视频模型请求只能包含一个 speaker，但字幕仍按该 speaker 的自然短句切换
- 每页 turns 的估计时长总和应接近页目标，并拆成适合字幕逐句切换的自然短句
- 讲解要求适用于所有页面，但不得替换或歪曲已有 notes 的核心内容
- 不设置每条发言或总讲稿时长硬上限
...
### Decision: 页目标时间轴使用模型合法分段和成片质量门禁

时间轴规划器以 `secondsPerSlide` 为每页最长播放窗口，并复用视频模型实际暴露的 `durationOptions` 计算内部片段。例如模型只支持 5/10 秒时，30 秒页面拆为三个 10 秒请求，7 秒页面可请求 10 秒但最多只合成前 7 秒。不得发送模型未声明支持的任意时长，也不得用供应商返回的自然结束时间扩大用户指定的页时间窗；短片提前结束时以实际时长为准。

讲稿按自然句拆分到片段和字幕 cue，片段逐个生成、缓存、检测和合成；同一页的多个片段共享原 PPT 快照，字幕随当前 cue 更新。每个片段进入最终录制前必须具有可解码、可播放的音轨。解码或播放失败时保留明确诊断并终止最终交付；片段提前结束时直接按实际时长收尾，不用静音填充或无限延长 PPT 画面。

### Decision: 能力与凭据在副作用前预检

预检根据来源验证文本、图片和视频模型的当前配置、API key 与可执行路由。任何必要条件缺失时，媒体缓存写入、生成调用和根任务持久化次数必须为零；静态音轨能力标签不再作为准入条件。
...
同一 PPT 的多个任务可以同时存在；内部媒体处理可排队或采用小并发，这是资源背压，不是产品拒绝规则。

### Decision: 只有最终根任务对素材库可见

增加显式任务结果可见性，例如 `resultVisibility: 'user' | 'internal'`。页面快照、讲稿和供应商中间结果均为 internal；任务存储读取和 Cache Storage 补充路径都必须过滤 internal。
...
- 部分现有视频模型可能忽略语音提示或返回无可用音轨的片段
  - Mitigation: 只展示供应商实际获取并勾选的模型，不宣称音轨能力；双人 turns 按角色独立请求并使用互斥的男声/女声约束；缓存、解码或播放失败时停止合成并保留真实错误。浏览器无法在提交前可靠预判供应商成片质量，用户需选择实际支持有声视频的模型
...
2. 新建 DTO、MCP schema 和 UI 只接受主题或当前 PPT，不再暴露 PPTX、历史 provider、声音或数字人字段；历史任务读取与恢复代码保持兼容
