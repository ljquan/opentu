...
新任务不声明或伪造额外的语音、数字人或专用成片服务。编排器把每页结构化 speaker turns 写入提示词，串行调用用户已选的视频模型生成带语音片段；不传 `referenceImages`，以复用普通文生视频请求链路，避免把请求切换成供应商未必支持的图生视频。PPT 模型选择器直接使用运行时供应商目录，只展示当前启用供应商实际获取并勾选的视频模型；不追加未获取的静态目录模型，也不再用静态音轨能力标签隐藏真实配置模型。
