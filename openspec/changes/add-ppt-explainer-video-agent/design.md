...
模型是否执行语音提示、能否生成可播放音轨以及朗读与时长是否稳定，由供应商真实结果决定。系统不得把模型名称、目录描述或本地标签包装成能力保证；缓存、媒体解码或播放失败时保留真实错误，并提示用户改用能够生成有声音轨的视频模型。用户设置的每页秒数是最终页时间窗，不是任意写入供应商请求的单段时长；编排器必须根据所选模型真实 `durationOptions` 将一页拆为一个或多个合法片段，并在最后一段存在溢出时只使用目标时间窗内的媒体。

生成式视频的视觉输出不是 PPT 固定画面，禁止直接拼接为最终成片。每页片段只作为讲解音轨来源：先强制缓存为同源 internal 媒体，再由独立本地合成器通过 HTMLMediaElement 和 Web Audio 播放音轨，Canvas 全程绘制原 PPT 快照、字幕和转场，最终只登记录制出的合成结果。字幕按讲稿的自然短句依次切换，每个时刻只展示当前 cue，不把整页讲稿长期叠在一张 PPT 上。双人模式通过角色名称、发言顺序和“使用不同声线”的文字约束表达，不承诺固定音色或精确复刻。界面和任务诊断必须提示：实际音色仍由模型决定，但系统会拒绝无可用音轨、明显长前导静音、有效语音覆盖不足或与目标页时间窗严重偏离的片段，不再静默交付已知无声或时长失控的成片。
...
讲稿生成以 `PPTFrameMeta.notes` 或 PPTX notes 为主要依据，同时遵循用户的每页目标秒数和讲解要求。文本模型按自然普通话约每秒 4 个有效字符估算页级目标字数，备注不足以覆盖目标时长时围绕原意扩写，明显过长时保留重点并压缩；只有单人备注长度接近目标且没有额外讲解要求时才能直接复用而跳过模型。
...
时间轴规划器以 `secondsPerSlide` 为每页最终播放窗口，并复用视频模型实际暴露的 `durationOptions` 计算内部片段。例如模型只支持 5/10 秒时，30 秒页面拆为三个 10 秒请求，7 秒页面可请求 10 秒但只合成前 7 秒。不得发送模型未声明支持的任意时长，也不得用供应商返回的自然结束时间扩大用户指定的页时间窗。

讲稿按自然句拆分到片段和字幕 cue，片段逐个生成、缓存、检测和合成；同一页的多个片段共享原 PPT 快照，字幕随当前 cue 更新。每个片段进入最终录制前必须确认包含可解码音轨、有效语音能覆盖其分配窗口且前导静音和总时长在容差内。门禁失败时允许有界重试，仍失败则保留明确诊断并终止最终交付，不能用静音填充、无限延长 PPT 画面或把后段少量声音包装成完整讲解。
