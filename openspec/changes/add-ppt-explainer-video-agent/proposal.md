...
公开兔子源码目前只提供通用媒体任务，没有可安全硬编码的 PPT 讲解成片端点。因此本变更增加两种明确执行模式：优先使用声明 `ppt-explainer` 能力的远端最终成片服务；没有专用服务时，OpenTu 使用现有文本、图片、音频和视频模型完成编排，并通过通用 `/audio/speech` TTS 与本地合成器生成最终成片。
...
- 新增所有用户直接可用的内置 OpenAI 兼容 `audio/speech` TTS 路由，自动复用当前供应商账号并返回可缓存的音频二进制；不要求用户添加音频模型或配置 binding
- 新增本地成片模式：按页组合 PPT 快照、TTS 旁白、字幕和转场；数字人模式可使用现有视频模型生成片段后参与合成
...
当前公开源码和文档未提供真实 `ppt-explainer` submit/poll/cancel 契约。本变更 SHALL NOT 猜测或硬编码兔子私有路径；没有真实专用 binding 时 SHALL 使用明确的本地合成模式，而不是把普通 Seedance 等视频 binding 伪装成专用成片服务。
