...
OpenTu 已有 PPT 大纲、页面整图、演讲备注、视频任务、供应商路由、素材库和画布插入能力，但这些能力彼此独立，用户无法从主题或当前 PPT 一次完成讲稿、单/双人讲解和最终成片。

公开兔子源码目前只提供通用媒体任务，没有可安全硬编码的 PPT 讲解成片端点。因此本变更只使用 OpenTu 当前已有的文本、图片和有声视频模型：逐页生成有声片段，再以原 PPT 页面作为固定画面本地合成为最终成片。

## What Changes

- 新增内置 Agent Skill 与 MCP 工具 `generate_ppt_explainer_video`
- 支持从主题生成或当前画布 PPT 两种来源创建任务
- 支持确认大纲，以及用户确认警告后跳过大纲确认
- 支持单人讲解和双人对谈两种讲解模式，只配置讲解者名称
...
- 把本页讲稿交给用户选择的现有视频模型生成有声片段，丢弃模型画面，再以原 PPT 快照、可用音轨、字幕和转场确定性合成
- PPT 模型选择器分别展示当前启用供应商已获取、勾选且具有可执行路由的图片与视频模型，不混入未获取或无路由的静态目录模型
- 使用 Web Locks、执行尝试校验和取消墓碑协调同一任务的跨标签执行，并对最终画布交付单独加锁
- 最终成片只登记一次，并同时进入素材库和创建任务时所属画布
- 不设置 OpenTu 产品层页数、单条发言、总成片时长或同 PPT 活跃任务数硬限制；供应商和运行环境的真实错误保持可见
...
当前公开源码和文档未提供真实的 PPT 专用成片契约。本变更 SHALL NOT 猜测或硬编码私有路径，也不得在产品入口构造专用 Agent、TTS、声音、参考音频或数字人能力。旧任务所需的 provider 或 PPTX 兼容代码可以保留，但 PPTX 来源不向新用户开放，所有新任务 SHALL 只从主题或当前 PPT 进入明确的逐页有声视频生成链路。
