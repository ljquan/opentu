...
公开兔子源码目前只提供通用媒体任务，没有可安全硬编码的 PPT 讲解成片端点。因此本变更增加两种明确执行模式：优先使用声明 `ppt-explainer` 能力的远端最终成片服务；没有专用服务时，OpenTu 使用现有文本、图片和有声视频模型逐页生成片段，再合并为最终成片。
...
- 新增现有模型成片模式：把逐页快照与本页讲稿交给 Seedance 等有声视频模型生成片段，再按页合并；界面明确提示语音内容、音色、时长和逐页同步由模型决定，可能波动
