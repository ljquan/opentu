...
- 把逐页快照与本页讲稿交给明确声明音频生成能力的视频模型，丢弃模型重绘画面，再以原 PPT 快照、音轨、字幕和转场确定性合成
- PPT 模型选择器只展示当前已配置且适配器契约明确支持音频生成的模型，不根据名称、描述或未公开接口猜测能力
- 使用 Web Locks、执行尝试校验和取消墓碑协调同一任务的跨标签执行，并对最终画布交付单独加锁
- 新增浏览器 PPTX 安全解析与逐页快照能力，统一按页进入现有模型链路
...
当前公开源码和文档未提供真实的 PPT 专用成片契约。本变更 SHALL NOT 猜测或硬编码私有路径，也不得在产品入口构造专用 Agent、TTS、声音、参考音频或数字人能力。旧任务所需的兼容解析代码可以保留，但所有新任务 SHALL 只走明确的逐页有声视频生成链路。
