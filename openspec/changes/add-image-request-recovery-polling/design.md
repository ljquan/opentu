...
API 部署统一放行 `X-Request-Id`，可信 Tuzi 节点均可在正式同步图片 POST 中携带该头；请求保持直连，不引入同源代理。

## Goals / Non-Goals

- Goals:
  - 所有图片提交和恢复查询默认直连用户配置节点；可信同步图片 POST 携带稳定 Request ID。
...
可信 Tuzi 节点集合用于受控备用切换。只有 API 明确返回 `accepted=false,retryable=true` 时，图片 POST 才能携带相同 Request ID 切换一个可信备用节点；其他媒体请求和第三方地址继续保持原路由。

当图片正式请求携带 Request ID 时：

- 配置节点已兼容 CORS：直连该节点并附加 Request ID。
- 配置节点是可信节点：直连该节点并附加 Request ID。
- 配置节点不可信或请求路径逃逸到第三方绝对 URL：不附加 Request ID。
- 网络错误、5xx、HTML 或普通 429 都不得改写地址或切换节点；备用 POST 网络错误立即结束为结果未知，不继续轮换。

恢复资格以可信 provider base URL、同步图片 binding 和已持久化提交身份为准，不再要求正式 POST 最终携带 `X-Request-Id`。任务在 POST 前持久化实际选中的 binding；逃逸到第三方绝对 URL 的请求仍不得进入固定结果查询。

共享传输层对 GET、HEAD 和其他非图片方法只直连用户配置节点。图片 POST 仅按上述明确协议受控切换。

恢复 GET 同时携带 `request_id` 查询参数和相同的 `X-Request-Id` 请求头；其他 GET 仍移除任意大小写形式的该请求头。本地联调节点仅在它与当前部署显式配置的 Tuzi API 地址一致时启用该行为，且不加入公网备用节点集合。
...
- 原配置节点优先查询；网络错误、429 或 5xx 后才在本轮查询可信备用节点，下一轮重新从原配置节点开始。
...
- 原配置节点短暂不可用时，恢复查询可在本轮继续查询共享存储可见的可信备用节点；下一轮仍从原节点开始。
- 可信节点均接收 `X-Request-Id`；若恢复查询仍返回 `processing_or_not_found`，系统只读轮询到原截止时间，不自动重复 POST。
