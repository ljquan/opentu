...
- 生成图片将自身提示词显示为可按需复用的淡化候选；普通本地图保持空提示词
...
1. 用户单选一张生成图片或普通本地图
2. 任务栏根据图片矩形和视口计算吸附位置；生成图片保持真实 prompt 为空并显示历史 prompt 候选，普通本地图不显示候选
3. 切换到另一目标时清理上一目标的手动附件、知识库引用和来源任务上下文
4. 空输入按空格或首次回车只填入候选；直接输入或操作其他任务栏控件则取消候选
5. 同一元素元数据刷新保持当前输入及候选取消状态；切换元素后建立新目标自己的候选
6. 用户修改或复用 prompt 后提交
...
- 提示词候选与取消元素 ID 只保存在当前 React 会话，不新增持久化字段或历史列表
