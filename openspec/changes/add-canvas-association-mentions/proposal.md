...
- 在提示词原光标位置显示可删除的内联类型编号引用（如 `@图片1`、`@图片2`、`@视频1`、`@文本1`）；引用标签参与提示词语义，媒体二进制仍与文本状态分离
...
- 请求被发布为画布任务后固化本次引用 ID，并用明显的黑色关系线把每个仍存在的来源分别连接到新建的 `generation-anchor` 或 `workzone` 任务节点；确认回执期间切板时延迟到来源板恢复后补线
- 最终结果插入画布后，把同一批关系线的任务端点迁移到首个最终结果；移动端点时同步线端点，删除任一端时仅移除关系线
- 慢任务与并发长视频按提交草稿快照和工作流 ID 隔离，旧任务完成不得清除后续输入或迁移其它任务的关系线
...
- Affected code:
  ...
  - `packages/drawnix/src/services/long-video-chain-service.ts`
